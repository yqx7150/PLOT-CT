...
def train_pipeline(root_path):
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            mini_gt_size = int(mini_gt_size)
            gt_size = int(gt_size)

            if int(mini_batch_size) < int(batch_size):
                indices = random.sample(range(0, batch_size), k=mini_batch_size)
                lq = lq[indices]
                gt = gt[indices]

            if int(mini_gt_size) < int(gt_size):
            ...
            if progress_bar is not None:
                #current_loss = model.get_current_log().get('loss_pix', 0.0)  # 使用正确的损失键
                progress_bar.update(1)
                progress_bar.set_postfix(
                    Epoch=epoch,
                    LR=f"{model.get_current_learning_rate()[0]:.6f}",
                    #loss_pix=current_loss,
                    Time=f"{iter_timer.get_avg_time():.2f}s/iter"
                    # 移除对 Remaining 的显式设置，使用 tqdm 内置格式
                )
            # -------------------------------------------------------------
